...
- Real-time controller close (`IController.close()`)
...
In addition, when the real-time controller is used as a context manager, the end of the `with` block automatically calls `close()`. If the device is already disconnected, `close()` raises `ConnectionError`—catch it as well.
